...
try {
    ...
    echo "Creating scholarship workflow tables...\n\n";
    
    // 1. Scholarship approval stages table
    echo "1. Creating scholarship_approval_stages table...\n";
    $conn->exec("
    ...
    ");
    echo "   ✓ scholarship_approval_stages created\n";
    
    // 2. Committee votes table
    echo "2. Creating committee_votes table...\n";
    $conn->exec("
    ...
    ");
    echo "   ✓ committee_votes created\n";
    
    // 3. Audit log table
    echo "3. Creating scholarship_audit_log table...\n";
    $conn->exec("
    ...
    ");
    echo "   ✓ scholarship_audit_log created\n";
    
    // 4. LOA templates table
    echo "4. Creating loa_templates table...\n";
    $conn->exec("
    ...
    ");
    echo "   ✓ loa_templates created\n";
    
    // 5. School approval config table
    echo "5. Creating school_approval_config table...\n";
    $conn->exec("
        CREATE TABLE IF NOT EXISTS school_approval_config (
            id INT PRIMARY KEY AUTO_INCREMENT,
            school_id INT NOT NULL,
            admin_email VARCHAR(255) NOT NULL,
            committee_emails JSON NOT NULL,
            vp_email VARCHAR(255) NOT NULL,
            president_email VARCHAR(255) NOT NULL,
            committee_quorum INT DEFAULT 3,
    ...
    ");
    echo "   ✓ school_approval_config created\n";
    
    // 6. Email log table
    echo "6. Creating scholarship_email_log table...\n";
    $conn->exec("
    ...
    ");
    echo "   ✓ scholarship_email_log created\n";
    
    // 7. Insert default LOA template
    echo "7. Inserting default LOA template...\n";
    $stmt = $conn->prepare("
        INSERT IGNORE INTO loa_templates (template_name, template_type, template_content, variables) VALUES 
        (?, ?, ?, ?)
    ");
    
    $templateContent = "LETTER OF AGREEMENT - SCHOLARSHIP PARTNERSHIP
...
Effective upon final approval and signature of all parties.";
    ...
    $stmt->execute(['Standard Scholarship LOA', 'STANDARD', $templateContent, $variables]);
    echo "   ✓ Default LOA template inserted\n";
    
    // 8. Insert default school approval configuration
    echo "8. Inserting default school approval configuration...\n";
    $stmt = $conn->prepare("
        INSERT INTO school_approval_config (
            school_id, admin_email, committee_emails, vp_email, president_email, committee_quorum
        ) VALUES (?, ?, ?, ?, ?, ?)
    ...
    $committeeEmails = json_encode([
        "committee1@davaocentralcollege.edu.ph", 
        "committee2@davaocentralcollege.edu.ph", 
        "committee3@davaocentralcollege.edu.ph"
    ]);
    ...
    echo "   ✓ Default school approval config inserted\n";
    
    // 9. Create workflow summary view
    echo "9. Creating workflow summary view...\n";
    $conn->exec("
        ...
        GROUP BY s.id
    ");
    echo "   ✓ Workflow summary view created\n";
    
    echo "\n🎉 All workflow tables created successfully!\n";
    echo "\nWorkflow system is now ready for testing.\n";
